...
class IOPrimus {
    ...
    generateEncryptedString() {
        try {
            var params =
                'jwt=' + IO_DATA.token + '&commKey=' + IO_DATA._commKey;
    ...
    onOpen() {
        ...
        this.primus.on(
            'error',
            function (data) {
                this.callback('io.error', data);
            }.bind(this),
        );
        this.primus.on(
            'reconnect',
            function () {
                this.callback('io.reconnect');
            }.bind(this),
        );
        this.primus.on(
            'end',
            function () {
                this.callback('io.close');
            }.bind(this),
        );
    }
    onData(data) {
        ...
        var decryptstr = CryptoJS.AES.decrypt(
            data,
            CryptoJS.enc.Utf8.parse(IO_DATA._commKey),
            {
        ...
        this.callback(
            parsedData.cmd,
            parsedData.res || parsedData.rep,
            parsedData.code,
            parsedData.error || parsedData.msg,
        );
    }
    connect() {
